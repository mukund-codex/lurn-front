...
@Component({
  ...
  templateUrl: './quiz-answer-details-report.component.html',
  styleUrls: ['./quiz-answer-details-report.component.css']
})

export class QuizAnswerDetailsReportComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    this.activeRoute.queryParams.subscribe(params => {
			this.answer_form_id = params['answer_form_id'];
			});
  ...
  loadReportData() {
	...
	loadData(Obj) {
		...
		return this.reportsService.getQuizAnswerDetailsReport(Obj);
  }
  ...
  export() {
    ...
    const curDate = this.reportsService.getCurrentDate();
		const fileName = 'Quiz-Report-'+ curDate;
  ...
  pageChange(pageNumber) {
    this.router.navigate([], {
      ...
      queryParams: { page: pageNumber, campaign_id: this.campaign_id ,answer_form_id:this.answer_form_id}
    });
  }
  ...
  ngOnDestroy(): void {
	...
	openUrl(answer_form_id,type){
		
		this.router.navigate(['/'+type], { queryParams : { answer_form_id: answer_form_id,campaign_id:this.campaign_id } } )
