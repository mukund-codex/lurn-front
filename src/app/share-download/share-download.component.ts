...
@Component({
  ...
  styleUrls: ['./share-download.component.css']
})
export class ShareDownloadComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    const permissionSet = this.reportsService.checkPermissionExist(['share-download-report-export', 'export-campaign']);
  ...
  loadReportData(){
	...
	loadData(Obj) {
		...
		return this.reportsService.getShareDownload(Obj);
  }
  ...
  export() {
		...
		const fileName = 'Share-Download-Report-'+ curDate;
		this.reportsService.exportData(this.searchObj, fileName, 'report', 'share_download_report_export');
	}
