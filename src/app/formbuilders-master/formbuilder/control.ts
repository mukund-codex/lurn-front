...
export class control {
	...
	constructor(config, preview) {
		...
		if (!config.id) {
			if (config.name) {
				...
				config.id =
					'control-' + Math.floor(Math.random() * 10000000 + 1);
			}
		}
		this.id = config.id;
	...
	static register(types, controlClass, parentType = null) {
		...
		for (const type of types) {
			...
			if (type.indexOf('.') > -1) {
				...
				control.error(
					`Ignoring type ${type}. Cannot use the character '.' in a type name.`
				);
				continue;
			}
	...
	static getRegistered(type = false): Array<any> {
		const types = (<any>Object).keys(control.classRegister);
		if (!types.length) {
			return types;
		}
	...
	static getClass(type: any, subtype = null): any {
		...
		const controlClass =
			control.classRegister[lookup] || control.classRegister[type];
		if (!controlClass) {
	...
	static mi18n(lookup: any, args: any = null) {
		...
		const value =
			typeof i18n == 'object' ? i18n[lookupCamel] || i18n[lookup] : i18n;
		if (value) {
			return value;
		}
	...
	static active(type) {
		return (
			...
			this.definition.inactive.indexOf(type) == -1
		);
	}
