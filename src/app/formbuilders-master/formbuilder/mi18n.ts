...
export default class I18N {
	...
	static fromFile(rawText) {
		...
		for (let matches, i = 0; i < lines.length; i++) {
			...
			if (matches) {
				const value = matches[2].replace(/^\s+|\s+$/, '');
	...
	static processFile(response) {
		const rawText = response.replace(/\n\n/g, '\n');
	...
	static loadLang(locale) {
		...
		return new Promise(function(resolve, reject) {
			if (I18N.langs[locale]) {
				...
				const langFile =
					this.config.location + locale + I18N.config.extension;
				xhr.open('GET', langFile, true);
				xhr.onload = function() {
					if ((<any>this).status <= 304) {
						const processedFile = self.processFile(xhr.responseText);
