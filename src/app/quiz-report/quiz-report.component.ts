...
export class QuizReportComponent implements OnInit, AfterViewInit {
  ...
  loadReportData() {
	...
	loadData(Obj) {
		...
		return this.reportsService.getQuizReport(Obj);
  }
  ...
  ngOnDestroy(): void {
	...
	openUrl(quiz_id,type){
		
		this.router.navigate(['/'+type], { queryParams : { quiz_id: quiz_id,campaign_id:this.campaign_id } } )
