...
export class SectionComponent implements OnInit, AfterViewInit {
	...
	ngOnInit() {
		...
		/*this.activeRoute.queryParams.subscribe(params => {
			...
			this.sectionService.setCourseId(this.course_id);
		  });*/
		...
		const permissionSet = this.sectionService.checkPermissionExist(['export-section', 'remove-section', 'remove-all-section', 'create-section', 'modify-section']);
	...
	loadData(Obj) {
		...
		return this.sectionService.getAll(Obj);
	...
	delete(section_id) {
		...
		this.sectionService.delete(sectionData,section_id).subscribe(param => {
			if(param["success"]){
	...
	deleteSelected() {
		...
		this.sectionService.delete_all(params).subscribe(param => {
	...
	upload(file) {
		...
		this.sectionService.upload(uploadData).subscribe(
	...
	export() {
		...
		const curDate = this.sectionService.getCurrentDate();
		const fileName = 'Section-'+ curDate;
		this.sectionService.exportData(this.searchObj, fileName);
	}
...
/*	changeCampaign(campaign) {
		...
		this.router.navigate(['section'], { queryParams : { course_id:this.course_id,campaign_id: campaign.id } });
		this.sf.form.updateValueAndValidity();
	}*/
