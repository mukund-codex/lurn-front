...
@Component({
  selector: 'app-course-details-report',
  templateUrl: './course-details-report.component.html',
  styleUrls: ['./course-details-report.component.css']
})

export class CourseDetailsReportComponent implements OnInit, AfterViewInit {
  ...
  loadReportData() {
	...
	loadData(Obj) {
		...
		return this.reportsService.getCourseDetailsReport(Obj);
  }
  ...
  export() {
    ...
    const curDate = this.reportsService.getCurrentDate();
		const fileName = 'Course-Details-Report-'+ curDate;
  ...
  ngOnDestroy(): void {
	...
	openUrl(course_id,type){
		
		this.router.navigate(['/'+type], { queryParams : { course_id: course_id,campaign_id:this.campaign_id } } )
