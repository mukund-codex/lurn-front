...
export class FormComponent implements OnInit {
	...
	ngOnInit() {
		...
		this.activeRoute.params.subscribe(param => {
			...
			if (param.id) {
				this.inputData = this.templateService.getTemplate(param.id).subscribe(result => {
					if (result['data'][0].mapping.length) {
						...
						this.getCamps(editData['mapping'][0]['camp']['campaigns'][0].id);
						this.getTemplates(editData['mapping'][0]['camp']['modules'][0].id);

						this.inputService.getTemplateById(editData['id']).subscribe(templateData => {
							this.inputsData = templateData['data'][0].input;
						});

						this.inputService.getCampsFromCampaign(editData['mapping'][0]['camp']['campaigns'][0].id).subscribe(camps => {
							this.getModules(editData['mapping'][0]['camp'], camps['data']);
							this.getInputs(editData['id']);
						});
					}
				});
			} else {
	...
	create_form(data?: any): FormGroup {
		...
		if (data) {
			...
			this.getcampaign(data['mapping'][0]['camp']['modules'][0]['company_id']);
		}

		return this.fb.group({
			company_id: [data ? [data['mapping'][0]['camp']['campaigns'][0]['company']] : '', [Validators.required]],
			campaign_id: [data ? [data['mapping'][0]['camp']['campaigns'][0]] : '', [Validators.required]],
			camp_id: [data ? [data['mapping'][0]['camp']] : '', [Validators.required]],
			module_id: [data ? [data['mapping'][0]['camp']['modules'][0]] : '', [Validators.required]],
			template_id: [data ? [data['id']] : '', [Validators.required]],
	...
	getInputs(template_id, fields?: any) {
		const that = this;
		this.templatesData.subscribe(result => {
			let data = result.data.map(arrayElement => {
				if(typeof template_id === "object"){
					template_id = template_id.id;
				}
				if (arrayElement['id'] === template_id) {
	...
	getTemplates(module_id?: string) {
		if (typeof module_id === 'object') {
			module_id = module_id['id'];
		}
		if (module_id) {
			this.templatesData = this.inputService.getTemplatesFromModule(module_id);
		}
	}
	...
	onSubmit() {
		...
		this.inputForm.value.template_id = this.inputForm.value.template_id[0].id;
	...
	private save() {
		this.inputService.save(this.formData).subscribe(
			(response: any) => {
				this.router.navigate(['/template-mapping']).then(() => {
